...
class NauticalMapLayer(context: Context) : OsmandMapLayer(context) {
    ...
    override fun onDraw(canvas: Canvas, tileBox: RotatedTileBox, settings: DrawSettings) {
        ...
        val settings = (context.applicationContext as net.osmand.plus.OsmandApplication).settings

        if (settings.NAUTICAL_SHOW_TRAJECTORY.get()) {
            ...
            if (size >= 2) {
                ...
                for (i in 0 until limit) {
                    ...
                    val alpha = ((i.toFloat() / limit) * 255).toInt()
        ...
        if (settings.NAUTICAL_SHOW_WIND_SHIFTS.get()) {
