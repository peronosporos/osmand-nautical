...
class NauticalRemoteServer(
            ...
            scope.launch {
                while (isRunning && !server.isClosed) {
                    try {
                        ...
                        if (!isRunning || server.isClosed) break
                        log.error("Error accepting connection: ${e.message}", e)
                    }
                }
            }
        } catch (e: Exception) {
    ...
    private fun handleClient(socket: Socket) {
        socket.use { s ->
            try {
                ...
                when {
                    path == "/api/telemetry" -> serveTelemetry(output)
                    else -> serveDashboard(output)
                }
            } catch (e: Exception) {
    ...
    private fun serveDashboard(output: OutputStream) {
    ...
    private fun serveTelemetry(output: OutputStream) {
